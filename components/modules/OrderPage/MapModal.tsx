...
import { INodoorsAddressData } from '@/types/order'
...
const MapModal = () => {
  ...
  const handleLoadMap = async (initialContainer = pickUpMapRef) => {
    ...
    const setMarkersByLocationsData = (data: INodoorsAddressData[]) => {
      data.forEach((item) => {
    ...
    if (!!chosenPickupAddressData.address_line1) {
      const chosenItem = nodoorsDataByCity.filter(
    ...
    if (nodoorsDataByCity.length) {
      setMarkersByLocationsData(nodoorsDataByCity)
    }
